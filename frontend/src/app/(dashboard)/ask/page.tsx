...
export default function AskPage() {
    ...
    const renderResultContent = (data: any) => {
                                ...
                                <div className="space-y-1">
                                    {Object.entries(item).map(([key, value]) => (
                                        <div key={key} className="flex flex-col sm:flex-row sm:gap-4 md:items-baseline">
                                            <span className="text-xs font-bold uppercase tracking-widest text-slate-500 w-32 shrink-0">{key}:</span>
                                            <span className="text-sm text-slate-900">{String(value)}</span>
                                        </div>
                                    ))}
                                </div>
                            ) : (
                                <span className="text-sm text-slate-900">{String(item)}</span>
                            )}
                        </li>
                    ))}
                </ul>
        ...
        if (typeof data === "object") {
            ...
            if (data.answer) {
                return <p className="text-sm text-slate-800 leading-relaxed whitespace-pre-wrap">{data.answer}</p>;
            }
                ...
                <div className="space-y-6">
                    {Object.entries(data).map(([sectionTitle, sectionContent]) => (
                        <div key={sectionTitle}>
                            <h3 className="text-xs font-bold uppercase tracking-widest text-slate-900 mb-3 border-b border-slate-200 pb-1">
    ...
    return (
        <div className="space-y-8 pb-12 max-w-4xl">
            ...
            <Card className="rounded-sm shadow-none border-slate-300 bg-white">
                <CardContent className="p-6">
                    <form onSubmit={handleQuery} className="space-y-6">
                        <div className="space-y-3">
                            <label
                                htmlFor="querySelect"
                                className="block text-xs font-bold uppercase tracking-widest text-slate-900"
                            >
                                Select System Query
                            </label>
                            <select
                                ...
                                className="flex h-10 w-full rounded-sm border border-slate-300 bg-slate-50 px-3 py-2 text-sm focus-visible:outline-none focus-visible:border-slate-900 focus-visible:ring-0 text-slate-900"
                                disabled={loading}
                            >
                        ...
                        <Button
                            type="submit"
                            disabled={loading}
                            className="w-full sm:w-auto rounded-sm bg-slate-900 text-white hover:bg-slate-800 h-10 font-bold tracking-widest uppercase px-8"
                        >
                            {loading ? "Compiling Report..." : "Execute Query"}
                        </Button>
                    </form>
                </CardContent>
            </Card>
