...
// Actual center coordinates for labels (calibrated to SVG paths)
const ZONES_CENTERS: Record<string, { x: number; y: number }> = {
    "North Delhi": { x: 160, y: 80 },
    "West Delhi": { x: 130, y: 150 },
    "Central Delhi": { x: 230, y: 175 },
    "East Delhi": { x: 275, y: 135 },
    "South Delhi": { x: 200, y: 250 },
};
...
export function DelhiMap({ issues, userZone }: DelhiMapProps) {
    ...
    const zoneData = useMemo(() => {
        ...
        ZONES.forEach(z => { data[z.id] = { totalScore: 0, count: 0, avg: 0 }; });
    ...
    const getZoneFill = (zoneId: string, avgScore: number) => {
        if (userZone && userZone !== "All Delhi" && userZone !== zoneId) {
            return { fill: "#f1f5f9", stroke: "#e2e8f0", glow: "none" };
        }
        if (avgScore === 0) return { fill: "#f1f5f9", stroke: "#cbd5e1", glow: "none" };
        if (avgScore > 55) return { fill: "url(#gradCritical)", stroke: "#dc2626", glow: "rgba(239,68,68,0.4)" };
        if (avgScore >= 45) return { fill: "url(#gradWarning)", stroke: "#d97706", glow: "rgba(245,158,11,0.4)" };
        return { fill: "url(#gradHealthy)", stroke: "#059669", glow: "rgba(16,185,129,0.4)" };
    };

    return (
        <Card className="rounded-xl border-slate-200 dark:border-slate-700/50 overflow-hidden shadow-sm">
            <CardContent className="p-4 sm:p-6 flex flex-col md:flex-row items-center gap-6">
                {/* SVG MAP */}
                <div className="relative shrink-0 flex items-center justify-center p-4 bg-gradient-to-br from-slate-50 via-slate-100 to-slate-50 dark:from-slate-800 dark:via-slate-900 dark:to-slate-800 rounded-xl border border-slate-200/50 dark:border-slate-700/30">
                    <TooltipProvider delayDuration={100}>
                        <svg viewBox="0 0 340 340" className="w-[280px] h-[280px] sm:w-[320px] sm:h-[320px]">
                            <defs>
                            ...
                            {ZONES.map((zone) => {
                                ...
                                const colors = getZoneFill(zone.id, avgScore);
                                const center = ZONES_CENTERS[zone.id];
                                const issueCount = zoneData[zone.id].count;
                                        ...
                                        <TooltipTrigger asChild>
                                            <g
                                                ...
                                                style={{
                                                    ...
                                                    transform: isHovered && isAccessible ? "scale(1.04)" : "scale(1)",
                                                    filter: isHovered && isAccessible ? "url(#zoneHoverGlow)" : "url(#zoneShadow)",
                                                }}
                                            >
                                                <path
                                                ...
                                                {isAccessible && center && (
                                                    <>
                                                        ...
                                                        <text
                                                            ...
                                                            y={center.y + 8}
                                                            textAnchor="middle"
                                                            fill={avgScore > 55 ? "#fca5a5" : avgScore >= 45 ? "#fcd34d" : "#6ee7b7"}
                                                            fontSize="8"
                                                            fontWeight="800"
                                                            style={{ pointerEvents: "none" }}
                                                        >
                                                ...
                                                {/* Pulsing dot for zones with active complaints */}
                                                {isAccessible && issueCount > 0 && avgScore > 50 && (
                                                    <>
                                                        <circle cx={center.x + 20} cy={center.y - 10} r="3" fill="#ef4444">
                                                            <animate
                                                                attributeName="r"
                                                                values="3;7;3"
                                                                dur="2s"
                                                        ...
                                                        <circle cx={center.x + 20} cy={center.y - 10} r="2.5" fill="#ef4444" />
                                                    </>
                                                )}
                                            </g>
                                        </TooltipTrigger>
                                        {isAccessible && (
                                            <TooltipContent className="bg-slate-900 text-white border-0 shadow-2xl px-4 py-3 rounded-xl" sideOffset={5}>
                                                <p className="text-xs font-bold uppercase tracking-widest text-slate-300 mb-1">{zone.name}</p>
                                                {avgScore > 0 ? (
                                                    <>
                                                        ...
                                                        <p className="text-xs text-slate-400 mt-0.5">{issueCount} active issues</p>
                                                    </>
                                                ) : (
                                                    <p className="text-sm text-slate-400">No active issues</p>
                                                )}
                                                <p className="text-[10px] text-violet-400 mt-1 font-medium">Click to drill down →</p>
                                            </TooltipContent>
                                        )}
                                    </Tooltip>
                                );
                    ...
                    <div className="absolute top-4 left-4 flex items-center gap-1.5">
                        ...
                        <span className="text-[10px] uppercase font-bold tracking-widest text-slate-400 dark:text-slate-500 pointer-events-none">
                            Delhi NCR · Live
                        </span>
                    </div>
                </div>
                ...
                <div className="flex-1 space-y-4">
                    <div>
                        <h3 className="text-sm font-bold uppercase tracking-widest text-slate-900 dark:text-slate-100">Regional Risk Heat Map</h3>
                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Geographic distribution of priority severity. Click any zone to drill down. Pulsing dots indicate active critical complaints.</p>
                    </div>

                    <div className="space-y-2 pt-2 border-t border-slate-100 dark:border-slate-700/50">
                        <div className="flex justify-between items-center text-xs">
                            <span className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-gradient-to-r from-red-600 to-rose-500"></div> Critical Risk (&gt;55)</span>
                            <span className="font-mono text-slate-500 dark:text-slate-400">
                                {ZONES.filter(z => (!userZone || userZone === "All Delhi" || userZone === z.id) && zoneData[z.id].avg > 55).length} zones
                            </span>
                        </div>
                        <div className="flex justify-between items-center text-xs">
                            <span className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-gradient-to-r from-amber-500 to-orange-400"></div> Elevated Risk (45-55)</span>
                            <span className="font-mono text-slate-500 dark:text-slate-400">
                                {ZONES.filter(z => (!userZone || userZone === "All Delhi" || userZone === z.id) && zoneData[z.id].avg >= 45 && zoneData[z.id].avg <= 55).length} zones
                            </span>
                        </div>
                        <div className="flex justify-between items-center text-xs">
                            <span className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-gradient-to-r from-emerald-500 to-teal-400"></div> Stable (&lt;45)</span>
                            <span className="font-mono text-slate-500 dark:text-slate-400">
                        ...
                        <div className="flex justify-between items-center text-xs opacity-50">
                            <span className="flex items-center gap-2"><div className="w-3 h-3 bg-slate-100 dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-full"></div> No Data / Restricted</span>
